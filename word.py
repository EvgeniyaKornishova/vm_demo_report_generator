...
def append_content(paragraph, content: Any):
    if isinstance(content, list) or isinstance(content, tuple):
        ...
        record = create_text_record(
            text=content.text, bold=content.bold, text_color=content.text_color, highlight_color=content.highlight_color
        )
...
def create_text_record(text: str, bold: bool = False, text_color: str = "#242424", highlight_color: str = None):
    ...
    if text_color is not None:
        etree.SubElement(
            style, etree.QName(W_NS, "color"), {etree.QName(W_NS, "val"): text_color}
        )

    if highlight_color is not None:
        etree.SubElement(
            style, etree.QName(W_NS, "highlight"), {etree.QName(W_NS, "val"): highlight_color}
        )
